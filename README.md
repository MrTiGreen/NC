...
## Диалоговые тест-боты

Для ручной проверки и smoke-теста чата есть два изолированных игрока: `Тестовый Разведчик` и `Тестовый Лекарь`. Они создаются только вне production и прогоняют короткий диалог через общий, гильдейский и личный чаты. Runner проверяет JWT-доступ, REST-создание сообщений, Socket.IO-доставку и обе стороны истории личного диалога.

Сначала запустите API и базу данных, затем в отдельном терминале выполните:

```bash
npm run bots:dialogue
```

Перед каждым прогоном runner очищает предыдущие сообщения этих двух ботов и создаёт свежую переписку, чтобы её можно было увидеть в интерфейсе. Чтобы удалить данные сразу после успешной проверки:

```bash
npm run bots:dialogue:cleanup
```

Для API на другом адресе передайте `api-url`, например `npm run bots:dialogue -- api-url=http://localhost:4001`.
